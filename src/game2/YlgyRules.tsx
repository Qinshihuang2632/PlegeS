...
export const YLGY_RULE_SECTIONS = [
    ...
    },
    {
        title: "AI 单词检测",
        points: [
            "填满的单词若与本局参考答案不同,AI(DeepSeek)会检测它是否为真实存在的英语单词;",
            "是真实单词 → 会告诉你它的释义,但提示它不符合本局参考答案、可能导致交叉单词无法填出;该词不会被锁定,你可以删掉换成本局答案;",
            "不是真实单词 → 按填错处理,扣 1 点血并标红;",
            "AI 暂时不可用时,自动回退为词库比对。",
        ],
    },
    {
        title: "草稿模式",
        points: [
            "拿不准某个字母?打开「草稿模式」后输入的字母只以小字写在格子角落,不会触发判定;",
            "草稿可以随时改写,或关闭草稿模式后在同格正式填写(正式填写才会触发检测);",
            "擦除会同时清掉草稿;正式填对的词不受草稿影响。",
        ],
    },
    {
        title: "道具(每局有限次数)",
...
export function YlgyRules({ compact = false }: { compact?: boolean }) {
    return (
        <div className={compact ? "space-y-3" : "space-y-5"}>
            ...
            <p className="pt-2 text-center text-[11px] text-muted-foreground">英了个语 · {YLGY_VERSION}(仅供个人娱乐)</p>
        </div>
    );
}