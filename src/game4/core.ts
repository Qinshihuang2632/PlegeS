 ...
 * 判胜: 本局 20 张物质全部被正确分类。
 * 难度: 简单(类别均衡抽样, 间隔 9s) / 标准(均衡 + 30% 掺入易错物质, 间隔 7s)
 *       / 困难(全库 259 种随机, 间隔 5s)。
 */
...
/** 出牌间隔(秒): 也即满载后留给玩家的清理时间 */
export const FLGL_INTERVAL: Record<FlglMode, number> = { easy: 9, normal: 7, hard: 5 };
...
/** 推进 dt 秒: 计时 + 出牌时刻处理(满载判负 / 出牌并重置间隔) */
export function tick(st: FlglState, dt: number): FlglState {
    ...
    return { ...st, phase, loseReason, spawnIn, spawned, belt, nextId, elapsed };
}
