...
> 生成时间:2026-08-23(配了个平 v1.0.0 上线后,诗了个句开发中)
...
3. `src/game6/SlgjPage.tsx` —— 游戏页:
   ...
   - 提交 POST `/slgj/api/rank` {mode,name,score,time,tools,version,platform};skipRank 逻辑同其它游戏
4. `functions/slgj/api/rank.js` —— 复制 plgp 模板改键:KV `slgj:`、rl `slgj:rl:`、score clamp ≤8、排序 score↓→time↑→tools↑
5. 前端接入:`public/_routes.json` 加 `/slgj/api/*`、`/slgj`;`App.tsx` 加 `/slgj` 路由;**HubPage 用 slgj 卡片替换最后一个「敬请期待」占位(占位归零,主界面 6 卡:5 可玩 + 错了个字维护中)**;玩法介绍弹窗加第六子页;RankPage 加第六 tab(PLGP 同款模式/得分+技能列/footer SLGJ_VERSION)
6. 后台:`functions/admin/api/rank.js` GAMES 加 slgj(复用 score/time/tools 排序);admin RanksPage、DashboardPage 加 slgj
7. `scripts/api_selftest.mjs` —— 12.58 段:slgj 提交/排序/时限 400/clamp/后台可见
8. 版本:`src/version.ts` PLATFORM_VERSION **v2.7.1 → v2.8.0**
9. 全量测试 + `npm run test:api` + build + 部署 + 线上验证(bundle 版本号/新卡片文案「诗了个句」/`/slgj` 200/`/slgj/api/rank` 空榜/后台 slgj 键)
10. 文档:`docs/slgj_summary_report.md`(新建,正序)+ `platform_log.md` 两行(slgj v1.0.0、平台 v2.8.0)+ 当前状态更新 + CONVENTIONS(六线→**七线**版本表/缩写加 slgj/KV 加 slgj:*/路由白名单加 /slgj/排序加 slgj)+ HANDOFF_PROMPT(概览表加行/目录加 game6+functions/slgj+docs 两份/新章节「配了个平→诗了个句」并重编号后续章节/KV 行/路由行/待办更新)+ roadmap #10 标记已上线 + docx 全部重生成
11. 弹窗确认(代理+实际检查)后 git 提交推送
