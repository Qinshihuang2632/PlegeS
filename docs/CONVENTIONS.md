...
5. 缩写固定:化了个学=hlgx、英了个语=**ylgy**(v1.4.9 起,由历史遗留的 `ws` 改名;旧链接 `/ws`、旧参数 `?game=ws` 重定向兼容)、错了个字=clgz、分了个类=flgl、配了个平=plgp、诗了个句=slgj(开发中)、**历了个史=llgs**(v2.9.0 上线)。
...
5. **贡献人双处同步约定(2026-08-30 确立, 2026-09-05 强化)**:此后所有**非本人(ps)贡献人**对更新有贡献时,必须在**日志(贡献人列/版本条目)**与**主界面「特别鸣谢」弹窗**(`src/game/HubPage.tsx` 内硬编码列表)两处同时同步 Ta 的贡献——新增条目或在既有条目中补充本次游戏版本与内容,缺一不可,不得只记日志。
...
## 常规更新路径(2026-09-05 用户确立)

此后每一次更新的标准流程(四步, 顺序固定):

1. **改代码**:在用户要求下完成代码修改与自测(全量测试通过);
2. **实机确认**:向用户确认「已实机测试更新已实现」(用户实际体验/检查通过);
3. **玩家版更新日志**:生成一版**面向玩家的更新日志文本**(写入 `src/game/updateNotice.ts` 的 `UPDATE_ITEMS`, 玩家视角措辞, 不写内部术语)交用户确认;
4. **git 同步**:在 2、3 均确认后, 提醒用户打开临时代理, 完成提交推送, 完成后清除代理配置。

配套机制:**更新日志弹窗**(v2.9.3)——玩家打开平台时, 若 `UPDATE_ID`(平台+各游戏版本拼接)较 localStorage 记忆有变化, 弹出一次更新日志, 可 ✕/「知道了」关闭, 关闭后不再弹(直到下次更新); 弹窗数据源即第 3 步确认的 `UPDATE_ITEMS`。另注意:用户未发指令前**不需要主动 git 同步、不需要弹窗询问代理**(2026-09-05 用户约定)。

---

## AI 检测接入约定(2026-08-30 起, 用户确立)

英了个语 v1.6.0 起引入 AI 单词检测, 管理后台「AI 检测」页可实时配置。接入约定(代码内置于 `functions/_lib/aicheck.js`, 改动前必读):

1. **配置来源优先级**:KV `ylgy:ai:config`(管理后台实时修改, 对局中即时生效, 无需重新部署) → 回退环境变量 `DEEPSEEK_API_KEY`(初始部署用)。
2. **Provider 映射**:新 AI 提供商必须兼容 OpenAI `/chat/completions` 协议(baseURL + 默认模型), 在 `AI_PROVIDERS` 表登记后即可在管理后台选择;不兼容的协议拒绝接入。
3. **Prompt 契约(固定)**:输入单词 → 输出 JSON `{"is_word": true|false, "pos": "词性缩写", "zh": "中文释义 ≤18 字"}`。
4. **响应契约(固定, 前端依赖)**:`detectWord()` 统一返回 `{ok:true, isWord, pos?, zh?}` 或 `{ok:false, msg}`——`ok:false` 时前端自动回退词库判定(旧行为), 保证 AI 不可用不阻塞游戏。
5. **实时性约定**:对局中玩家每填满一个「非参考答案」词即实时异步调用(不阻塞输入);15 秒超时;同词结果 KV 缓存 24h;每 IP 每分钟限频 20 次。
6. **安全**:API Key 只存 KV/环境变量, 不下发前端;管理端展示一律脱敏(前4+****+后4);每次保存/测试写审计日志(`ai_config_update` / `ai_config_test`)。
7. **游戏侧判定契约(ylgy v1.6.0)**:填满单词先与生成时参考答案比对(一致 → 变绿锁定, 零延迟);不同才走 AI——真实单词 → 仅提示释义不锁定可删改;非真实单词 → 扣血红 2 秒;AI 不可用 → 回退词库判定。
...
| 2026-08-30 | 非 ps 贡献人的贡献必须同步到主界面「特别鸣谢」弹窗, 不得只记日志 | 楠鸢贡献(题库扩容/分了个类调整)同步时确立 |
| 2026-09-05 | 贡献人双处同步强化: 日志贡献人列与特别鸣谢栏必须同时更新, 缺一不可 | 历了个史 v1.0.0 wcx 贡献时强化 |
| 2026-09-05 | 常规更新路径四步(改码→实机确认→玩家版日志确认→代理同步) + 玩家端更新日志弹窗 | 更新弹窗上线(平台 v2.9.3) |
| 2026-08-30 | AI 检测接入约定(provider 映射/prompt 与响应契约/KV 配置/降级/安全) | 英了个语 v1.6.0 + 管理端 AI 设置页(平台 v2.8.4) |
