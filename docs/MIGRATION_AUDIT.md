...
- **违禁词扩充**:新增"违反党史国情/国家大政方针"类(台独/港独/藏独/疆独/蒙独/民族分裂/分裂国家/
  ...
  前后端双份同步,后端 400 强制
- **手游/端游榜单分离**:提交带 `platform`(前端 `detectPlatform`:pointer:coarse → maxTouchPoints → UA),
  后端缺省按 User-Agent 兜底;条目存 platform 字段,GET `?platform=mobile|desktop` 过滤,
  旧条目(无 platform)归端游;**surpassed 只与同平台比较**;游戏页状态栏显示平台徽标、
  榜单页新增端游/手游切换、管理端榜单表格新增平台列
- **验证**:Vitest 32/32(含违禁词单元)、契约 79/79(新增平台 8 项)、E2E 37/37;
  生产实测:版本号 v2.1.4 上线、mobile 提交与过滤生效、英文违禁词 400 拒绝
- **E2E 幂等性修复**:本地 KV 持久化导致同名限频/登录锁定跨轮累积,
  所有测试昵称与模拟 IP 改为每轮时间戳唯一
